...
export default function ResetPassword() {
  ...
  const otp = queryParams.get("otp"); // passed from VerifyResetOTP
  ...
  const handleReset = async () => {
    ...
    setLoading(true);
    try {
      // v2: reset-password now requires email + otp + new_password
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-sky-300 via-blue-300 to-indigo-400 relative overflow-hidden">
      ...
      <div className="relative backdrop-blur-xl bg-white/30 border border-white/40 shadow-2xl rounded-3xl p-10 w-96">
        <div className="flex justify-center mb-6">
          <div className="bg-white/80 shadow-md p-4 rounded-2xl text-xl">🔐</div>
        </div>
        <h2 className="text-2xl font-bold text-center text-gray-800 mb-2">Reset Password</h2>
        <p className="text-center text-gray-600 text-sm mb-6">Create a new secure password.</p>
        ...
        <input
          ...
          className="w-full p-3 mb-6 bg-white/70 text-gray-800 border border-white rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none shadow-sm"
        ...
        <button
          onClick={handleReset}
          disabled={loading}
          className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 text-white py-3 rounded-xl font-semibold shadow-lg hover:scale-[1.02] transition disabled:opacity-50"
        >
